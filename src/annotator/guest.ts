...
/**
 * Return the element with the document data associated with any highlights that contain a given
 * DOM node.
 */
function getDataElement(anchorNode: Node) {
  if (
    !anchorNode.parentElement ||
    !anchorNode.parentElement.dataset ||
    !anchorNode.parentElement.dataset.documentId
  ) {
    ...
    if(
      ...
      !anchorNode.parentElement.parentElement.dataset ||
      !anchorNode.parentElement.parentElement.dataset.documentId
    ) {
      return null;
    }
    return anchorNode.parentElement.parentElement;
  }
...
/**
 * Return the dataset associated with any highlights that contain a given
 * DOM node.
 */
function getDocumentData(selection: Node) {
  const dataElement = getDataElement(selection.anchorNode);
...
export class Guest extends TinyEmitter implements Annotator, Destroyable {
  ...
  /** Document id from node of selection range. */
  public documentId: string;

  /** Service id from node of selection range. */
  ...
  _onSelection(range: Range) {
    ...
    const documentData = getDocumentData(selection);
